...
fn test(blocks_dir: &str, filter_file: &str) -> Result<()> {
    ...
    println!("\nTesting parse_o");
    let test_block = BlockHash::from_str("0000000000000aff9f826ed90c99e0aeb673e22494e830fecde4113a6bc264af")?;
    let test_txid = Txid::from_str("3045a3b13be965622fb5d1652a810e39dca28913f495604f99de6ffb9f71f587")?;
    let parser = BlockParser::new(&locations);
    ...
    println!("\nTesting parse_i");
    let test_txid = Txid::from_str("cf2cc1897eb061e2406e644ecee3c26ee64cfadcc626890438c3d058511c9094")?;
    let parser = BlockParser::new(&locations);
    let amounts = vec![sat(56892597), sat(274000000), sat(248000000), sat(14832476), sat(48506744443)];
    for parsed in parser.parse_i() {
        let parsed = parsed?;
        for (_, txid) in parsed.transactions() {
            if *txid == test_txid {
